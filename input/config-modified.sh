...
# Add custom rules
echo "-w /home/student/secret/secret.txt -p rwxa -k secret_access" >> $audit_rule_path
echo "-w /home/student/downloads/ -p x -k program_execution" >> $audit_rule_path
echo "-a always,exit -F arch=b64 -S connect -F a0=2 -F a1=8080 -k network_activity" >> $audit_rule_path
echo "-a always,exit -F auid=$(id -u attacker) -S all -k attacker_activity" >> $audit_rule_path
echo "-a always,exit -F arch=b64 -S open,openat,read,write,execve -k syscall_monitoring" >> $audit_rule_path
